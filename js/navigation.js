...
(() => {
  ...
  /* ---- The logo ----
   * Its entrance, strands drawing together into the signature, is pure
   * CSS (see .sig in main.css). This only holds it: in the homepage header
   * until the intro film has cleared, so it arrives on a screen the
   * visitor can see, and in the footer until it scrolls into view.
   */
  {
    ...
    autoplaySigs.concat(viewSigs).forEach((sig) => sig.classList.add("is-waiting"));
    viewSigs.forEach((sig) => sig.classList.add("is-signing"));

    const intro = document.querySelector("[data-intro]");
    autoplaySigs.forEach((sig) => {
      if (intro && !intro.hidden && !intro.contains(sig)) {
    ...
    if (viewSigs.length && !("IntersectionObserver" in window)) {
      viewSigs.forEach(release);
    } else if (viewSigs.length) {
